...
# Contains declarations of media (only includes movies at the moment).
# The media instances are then put into an array,
# which they are then passed into the frozen tomatoes method,
# which dynamically generates a webpage with all of the declared media.

# Declare array that will contain all of the movies:
these_movies = []
...
up_film = media.Movie(
    "Up",
    "How do you make a grown man cry within 5 minutes? Up.<br>"
    "Arguably the best Pixar film to date.",
...
donnie_darko = media.Movie(
    "Donnie Darko",
    "'A storm is coming. A storm that will swallow the children. And I will "
    "deliver them from the kingdom of Bane. I'll deliver the children back to "
    "their doorsteps. I'll send the monsters back to the underground. I'll "
    "send them back to a place where no one else can see them except for me, "
...
american_history_x = media.Movie(
    "American History X",
    "I believe that you often develop a hatred for something because it so "
    "accurately defines who you are. You are convinced that you are utterly "
    "trapped in your circumstance, and you try to take pride in that mindset. "
    "And oftentimes, love is what breaks that hatred, and leads to "
    "transcendence. Here is a tragic, beautiful example, one that I look back "
    "to whenever I am mired in hatred.",
...
# Inserts all movies to the array
...
# Dynamically generates frozen tomatoes website with all the movies inside.
